...
export default function TaskList({ tasks, clientId }: { tasks: any[]; clientId: number }) {
    ...
    const now = new Date();
    const dueTasks = tasks.filter((task) => !task.completed && new Date(task.dueDate) >= now);
                                        ...
                                        <div className={styles.taskNotesWrap}>
                                            ...
                                            <div className={styles.taskNotesList}>
                                                {task.notes.map((note: any) => (
                                                    <div key={note.id} className={styles.taskNoteItem}>
                                                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.5rem' }}>
                                                            <div style={{ flex: 1 }}>
                                                                <p className={styles.taskNoteText}>{note.content}</p>
